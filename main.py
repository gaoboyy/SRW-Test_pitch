...
def set_optics(v=None):
    ...
    intNx=512
    intNy=512
   #gridSize = (1/math.pi**0.5)*8.e-06  # [m]
    gridSize = 5.e-06  # [m]
    pitch_x = 25.1e-6
    pitch_y = 25.1e-6
    mask_x0=pitch_x/2
    ...
    hx = 5.86e-7   # [m]
    hy = 5.86e-7   # [m]
    grid_sh = 1    #grid shape 0. circle hartamann 1. rectangle hartamann 2. 2D grating
    # Generate a 2D Mask.
    opMask = srwl_opt_setup_mask(_delta=1, _atten_len=1, _thick=1, _grid_sh=grid_sh, _grid_dx=gridSize,
    ...
    el.append(srwlib.SRWLOptD(10))

    el.append(opMask)
    el.append(srwlib.SRWLOptD(0.4+1.e-06))
    pp = []
...
#---Calculation Types
    ...
    ['tr_ctf', 'f', 3.0, 'final time moment (c*t) for electron trajectory calculation [m]'],
    ...
    ['w_rx', 'f', 0.00015, 'range of horizontal position [m] for calculation of intensity distribution'],
    ['w_nx', 'i', 256, 'number of points vs horizontal position for calculation of intensity distribution'],
    ['w_y', 'f', 0.0, 'central vertical position [m] for calculation of intensity distribution vs horizontal and vertical position'],
    ['w_ry', 'f', 0.000225, 'range of vertical position [m] for calculation of intensity distribution vs horizontal and vertical position'],
    ['w_ny', 'i', 384, 'number of points vs vertical position for calculation of intensity distribution'],
    ['w_smpf', 'f', 0.0, 'sampling factor for calculation of intensity distribution vs horizontal and vertical position'],
