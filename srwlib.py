...
class SRWLOptG(SRWLOpt):
    ...
    def __init__(self, _grDen=100, _disPl='v', _ang=0.7854, _m=1, _refl=1, _grDen1=0, _grDen2=0, _grDen3=0, _grDen4=0):
        """
        :param _grDen: grove density [lines/mm] (coefficient a0 in the polynomial grove density: a0 + a1*y + a2*y^2 + a3*y^3 + a4*y^4)
        ...
        :param _grDen1: grove density polynomial coefficient a1
        :param _grDen2: grove density polynomial coefficient a2
        :param _grDen3: grove density polynomial coefficient a3
        :param _grDen4: grove density polynomial coefficient a4
        """
        self.grDen = _grDen #grove density [lines/mm] (polynomial coefficient a0 in a0 + a1*y + a2*y^2 + a3*y^3 + a4*y^4)
        self.grDen1 = _grDen1 #grove density polynomial coefficient a1
        self.grDen2 = _grDen2 #grove density polynomial coefficient a2
        self.grDen3 = _grDen3 #grove density polynomial coefficient a3
        self.grDen4 = _grDen4 #grove density polynomial coefficient a4
...
class SRWLOptMirPl(SRWLOptMir):
    ...
    def __init__(self, 
                 ...
                 _size_tang=1, _size_sag=1, _ap_shape='r', _sim_meth=2, _treat_in_out=1, _ext_in=0, _ext_out=0,
        ...
        self.set_dim_sim_meth(_size_tang, _size_sag, _ap_shape, _sim_meth, 100, 100, _treat_in_out, _ext_in, _ext_out)
...
def srwl_wfr_emit_prop_multi_e(_e_beam, _mag, _mesh, _sr_meth, _sr_rel_prec, _n_part_tot, _n_part_avg_proc=1, _n_save_per=100, _file_path=None, _sr_samp_fact=-1, _opt_bl=None, _pres_ang=0, _char=0, _x0=0, _y0=0):
    ...
    if(nProc <= 1):
        ...
        nPartPerProc = _n_part_avg_proc*nSentPerProc
    ...
    if((rank > 0) or (nProc == 1)):
        ...
        if((nProc > 1) and (_opt_bl != None)): #receive mesh for the resulting wavefront from the master
            ...
            #print("received mesh %d -> %d" % (_stat.Get_source(), rank)) #an he
        ...
        for i in range(nPartPerProc): #loop over macro-electrons
            ...
            if(iAvgProc >= _n_part_avg_proc):
                if(nProc > 1):
                    ...
                    #DEBUG
                    srwl_uti_save_text("Sent # " + str(iAuxSendCount), _file_path + "." + str(rank) + "as.dbg")
                    #END DEBUG
        ...
        for i in range(nRecv): #loop over messages from workers
            ...
            #DEBUG
            srwl_uti_save_text("Preparing to receiving # " + str(i), _file_path + ".0b.dbg")
            #END DEBUG
           
            comMPI.Recv([workStokes.arS, MPI.FLOAT], source=MPI.ANY_SOURCE) #receive #an he (commented-out)

            #DEBUG
            srwl_uti_save_text("Received intensity # " + str(i), _file_path + ".0.dbg")
            #END DEBUG
